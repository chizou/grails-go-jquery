...
*/
/*
    resp, err := client.Get("http://" + p.PIAZZA_PRIME_BOX + ":8081/service")
    if err != nil {
        fmt.Println("Something went wrong for pz1curl 8081")
        return false
    }
    defer resp.Body.Close()
*/
...
defer resp.Body.Close()
    ...
    fmt.Printf("Error: %v\n", err3)
    fmt.Printf("363: ")
    fmt.Printf(string(body3))
    return string(body3) //strings.Contains(string(body), `Piazza Service Controller`)
}
